...
#	echo "cache-file=/tmp/ez-ipupdate.cache" >>$IPUPDATE_CONF
	...
	### ECONET-1305 START ###
		elif [  "$SERVERNAME" = "$SERVER_VDDNS" ] ; then
			echo "service-type=vddns" >>$IPUPDATE_CONF
	### ECONET-1305 END ###
		else
		...
		echo "service-type=$SERVICETYPE" >>$IPUPDATE_CONF
		echo "server=$SERVERNAME" >>$IPUPDATE_CONF
	fi
	# ONT-583 END
