...
<%if tcWebApi_get("System_Entry","upgrade_fw_status","h") <> "SUCCESS" then%>
	<head>
<title>Viettel ONT - <%tcWebApi_get("String_Entry","NavigationMainFirmwareText","s")%></title>		<meta charset="utf-8">
...
function doUpgrade() {
	...
	var string = form.FW_UploadFile.value.search(/vG_ONU_/);
...
$(".custom-file-input").on("change", function() {
  ...
  var version = fileName.split("vG_ONU_");
  var string = "";
  if (version.length != 2) {
  ...
  string = "<%tcWebApi_get("String_Entry","NewFirmwareVersionText","s")%> V" + version + ". ";
