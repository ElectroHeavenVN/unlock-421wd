...
if Request_Form("defaultflag") = "1" then
	if Request_Form("restoreflag2") = "1" then 
		TCWebApi_set("System_Entry", "upgrade_fw", "HTML_HEADER_TYPE")
		TCWebApi_constSet("System_Entry", "reboot_type", "0")
		TCWebApi_CommitWithoutSave("System_Entry")
	end if
end if
if Request_Form("defaultflag") = "1" then
	if Request_Form("restoreflag2") = "2" then 
...
if Request_Form("defaultflag") = "1" then
	if Request_Form("restoreflag2") = "6" then 
		TCWebApi_set("System_Entry","reboot_type","restoreflag2") 
		TCWebApi_set("Account_Entry0","Logged","login_init")
		TCWebApi_commit("System_Entry")
	end if
...
<html>
	<head>
<title><%tcWebApi_get("String_Entry","BranchName","s")%> ONT - <%tcWebApi_get("String_Entry","NAVConfig","s")%></title>		<meta charset="utf-8">
		...
		function doReset() {
				...
				<%end if%>
				{
					document.ConfigForm.defaultflag.value = "1";
		...
		/*Recover - ONTS-150*/
		function doRecover(){
			if(confirm("<%tcWebApi_get("String_Entry","RecoverNotify","s")%>"))
			{
				document.ConfigForm.defaultflag.value = "1";
				document.ConfigForm.restoreflag2.value = "6";
				submitAsync("ConfigForm");
				setTimeout("doRefresh()",30*1000);
			}
		}
		/* Backup Config - ONTS-56*/
		function doBackup() {
			var cfg = '<% tcWebApi_get_romfile_enc_name() %>';
			location.assign(cfg);
			setTimeout(function(){ window.top.location.href = "/cgi-bin/tools-config.asp"; }, 3000);
		}
		/*Restore Config*/
		function doRestore(){
			var string = document.ConfigForm.customFile.value.search(/romfile/);
			if(document.ConfigForm.customFile.value=="") {
...
<% end if end if %>		<div id="content">
			<form class="form-inline" ENCTYPE="multipart/form-data" METHOD="POST" ACTION="/cgi-bin/tools-config.asp" name="ConfigForm" >
			<input type="hidden" name="TokenString" value="<%tcWebApi_getToken()%>">
			<div class="row row-w-100">
				<div class="widget col-xs-12 col-lg-12">
					<header><%tcWebApi_get("String_Entry","BackUpConfigText","s")%></header>
					<div class=widget-body>
						<div class="form-group">
							<label class="text-justify mb-2"><%tcWebApi_get("String_Entry","BackUpconfigNoteText","s")%></label>
						</div>
						<div class="col-lg-12 d-flex align-items-end justify-content-end">
							<button type="button" class="page-button small-button" onclick="doBackup();"><%tcWebApi_get("String_Entry","ButtonBackupText","s")%></button>
						</div>
					</div>
				</div>
				<div class="widget col-xs-12 col-lg-12">
					<header><%tcWebApi_get("String_Entry","RestoreConfigText","s")%></header>
					<div class=widget-body>
						<div class="form-group">
							<label class="text-justify mb-2"><%tcWebApi_get("String_Entry","RestoreConfigNoteText","s")%></label>
							<div class="custom-file col-lg-6">
								<input type="file" class="custom-file-input" id="customFile" name="customFile">
								<label class="custom-file-label" for="customFile"><%tcWebApi_get("String_Entry","ChooseFileText","s")%></label>
						...
						<%elseif tcWebApi_get("System_Entry","upgrade_fw_status","h") = "FAIL" then%>
							<div class="form-group">
								<label class="text-justify mb-2"><%tcWebApi_get("String_Entry","UpdateStatusText","s")%>: </label>
								<label class="text-justify mb-2" style="color:red">
									<%tcWebApi_get("String_Entry", "FileIllegalText", "s")%>
								</label>
								<%tcWebApi_constSet("System_Entry", "upgrade_fw_status", "NONE")%>
							</div>
						<%else%>
							<div class="form-group">
								<label class="text-justify mb-2"><%tcWebApi_get("String_Entry","UpdateStatusText","s")%>: </label>
								<label class="text-justify mb-2" style="color:green">
									<%tcWebApi_get("String_Entry", "UpdateSuccessfulMsgText", "s")%>
								</label>
							</div>
						<%end if%>
						<div class="form-group" style="min-height: 0px;">
							<label ID="warn" class="text-justify color-theme-red"></label>
						</div>
						<div class="col-lg-12 d-flex align-items-end justify-content-end">
							<button type="button" class="page-button small-button" onclick="doRestore();"><%tcWebApi_get("String_Entry","ButtonRestoreText","s")%></button>
						</div>					
					</div>
				</div>
				<!--ONTS-150-->
				<div class="widget col-xs-12 col-lg-12">
					<header><%tcWebApi_get("String_Entry","RecoverText","s")%></header>
					<div class=widget-body>
						<div class="form-group">
							<label class="text-justify mb-2"><%tcWebApi_get("String_Entry","RecoverExplain","s")%></label>
						</div>
						<div class="col-lg-12 d-flex align-items-end justify-content-end">
							<button type="button" class="page-button small-button" onclick="doRecover();"><%tcWebApi_get("String_Entry","RecoverText1","s")%></button>
						</div>
					</div>
				</div>
				<div class="widget col-xs-12 col-lg-12">
					<header><%tcWebApi_get("String_Entry","FactoryResetText","s")%></header>
					<div class=widget-body>
						<div class="form-group">
							<label class="text-justify mb-2"><%tcWebApi_get("String_Entry","FactoryResetNoteText","s")%></label>
						</div>
						<div class="col-lg-12 d-flex align-items-end justify-content-end">
							<button type="button" class="page-button small-button" onclick="doReset();"><%tcWebApi_get("String_Entry","ButtonFactoryText","s")%></button>
						</div>
					</div>
					<input type="hidden" id="defaultflag" name="defaultflag" value="0">
                    <input type="hidden" id="restoreflag2" name="restoreflag2" value="0">
					<input type="HIDDEN" name="HTML_HEADER_TYPE" value="2"> <!--upgrade_fw=2-->
				</div>
			</div>
			</form>
		</div>
