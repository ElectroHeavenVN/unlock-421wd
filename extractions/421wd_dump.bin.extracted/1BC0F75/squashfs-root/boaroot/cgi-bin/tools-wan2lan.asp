...
%>
<%
if Request_Form("wan2lan_Flag") = "1" then 
	TCWebApi_unset("wan2lan")
	TCWebApi_set("wan2lan","Enable","EnableWAN2LAN_Flag")
    TCWebApi_set("wan2lan","LAN1","bindlan1")
    TCWebApi_set("wan2lan","LAN2","bindlan2")
    TCWebApi_set("wan2lan","LAN3","bindlan3")
	TCWebApi_set("wan2lan","LAN4","bindlan4")
	TCWebApi_commit("wan2lan")
	TCWebApi_save()
end if
%>
<!doctype html>
<html>
	<head>
<title><%tcWebApi_get("String_Entry","BranchName","s")%> ONT - <%tcWebApi_get("String_Entry","Wan2lanText","s")%></title>		<meta charset="utf-8">
		...
		<script language="javascript" type=text/javascript>
			...
			function SubmitForm()
			{
				var vForm = document.wan2lan_form;
				...
				vForm.wan2lan_Flag.value = "1";
				submitAsync("wan2lan_form");
			}
            function setWAN2LAN()
			{
				var attackChecked = getCheckVal('enableWAN2LAN');
				console.log(attackChecked)
				if (attackChecked == '1')
				{
					document.wan2lan_form.EnableWAN2LAN_Flag.value = "Yes";
				}
				else
				{
					document.wan2lan_form.EnableWAN2LAN_Flag.value = "No";
				}
			}
		</script>
		<style>
...
<% end if end if %>		<div id="content">
			<form class="form-inline" METHOD="POST" ACTION="/cgi-bin/tools-wan2lan.asp" name="wan2lan_form" id="wan2lan_form" >
				<input type="hidden" name="TokenString" value="<%tcWebApi_getToken()%>">
				<input type="hidden" id="wan2lan_Flag" name="wan2lan_Flag" value="0">
				<input type="hidden" id="EnableWAN2LAN_Flag" name="EnableWAN2LAN_Flag" value="<% tcWebApi_get("wan2lan","Enable","s") %>">
				<div class="row row-w-100">
					<div class="widget col-sm-12">
						<header>Uplink Mirroring Setting </header> 
						<div class="widget-body">
							<div class="form-group">
								<label class="col-lg-5" for="enableWAN2LAN"><%tcWebApi_get("String_Entry","EnableText","s")%></label>
								<label class="toggle-switch">
									<input type="checkbox" name="enableWAN2LAN" id="enableWAN2LAN" onclick="setWAN2LAN()" <%if tcWebApi_get("wan2lan","Enable","h") = "Yes" then asp_Write("checked") end if%>>
									<!-- <input type="checkbox" name="enableWAN2LAN" id="enableWAN2LAN" onclick="setWAN2LAN()"> -->
									<span class="slider"></span>
								</label>
							</div>
							<div class="form-group d-flex" id="divlanport">
								<label class="col-lg-5 col-sm-2">Mirrored Port</label> <!--chiem 5 colume-->
								<div class="col-lg-7 d-flex"> <!--chiem 7 colume 5+7=12-->
									<div class="custom-control custom-checkbox col-lg-3" id=secLan1>
										<INPUT id=cb_bindlan1 type=checkbox name=cb_bindlan1 <%if tcWebApi_get("wan2lan","LAN1","h") = "Yes" then asp_Write("checked") end if%> class="custom-control-input" >
									...
									<div class="custom-control custom-checkbox col-lg-3" id=secLan2>
										<INPUT id=cb_bindlan2 type=checkbox name=cb_bindlan2 <%if tcWebApi_get("wan2lan","LAN2","h") = "Yes" then asp_Write("checked") end if%> class="custom-control-input" >
									...
									<div class="custom-control custom-checkbox col-lg-3" id=secLan3>
										<INPUT id=cb_bindlan3 type=checkbox name=cb_bindlan3 <%if tcWebApi_get("wan2lan","LAN3","h") = "Yes" then asp_Write("checked") end if%> class="custom-control-input" >
									...
									<div class="custom-control custom-checkbox col-lg-3" id=secLan4>
										<INPUT id=cb_bindlan4 type=checkbox name=cb_bindlan4 <%if tcWebApi_get("wan2lan","LAN4","h") = "Yes" then asp_Write("checked") end if%> class="custom-control-input" >
