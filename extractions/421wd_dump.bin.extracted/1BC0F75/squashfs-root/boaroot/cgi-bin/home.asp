...
TCWebApi_set("WLan11ac_Entry0","SSID5G_bndstr_off","SSID5G_bndstr_off")
TCWebApi_set("WLan11ac_Entry0","AuthMode5G_bndstr_off","AuthMode5G_bndstr_off")
TCWebApi_set("WLan11ac_Entry0","WPAPSK5G_bndstr_off","WPAPSK5G_bndstr_off")
...
if TCWebApi_get("WLan_Common","ToCommit","h") = "1" then
	TCWebApi_ConstSet("WLan_Mesh", "EasysetupWlanCommit", "1")
...
if TCWebApi_get("WLan11ac_Common","ToCommit","h") = "1" then
	TCWebApi_ConstSet("WLan_Mesh", "EasysetupWlan11acCommit", "1")
...
<html>
	<head>
<title><%tcWebApi_get("String_Entry","BranchName","s")%> ONT - <%tcWebApi_get("String_Entry","NavigationHomeText","s")%></title>		<meta charset="utf-8">
		...
		<script language="JavaScript" type="text/JavaScript">
			...
			var wSSID =  "<%tcWebApi_Get_JSEscape("WLan_Entry0","SSID","s")%>";
			...
			var w11acSSID = "<%tcWebApi_Get_JSEscape("WLan11ac_Entry0","SSID","s")%>";
			...
			// For check wifi scheudle
			function display_wifischedule_note() {
				var ws_running = "<%tcWebApi_get("wifischeduling.common","Running","s")%>";
				console.log('ws_running: ', ws_running);
				if (ws_running == '1') {
					$("#Enable_AccessPoint").prop("disabled",true);
					$("#wlan11ac_APenable").prop("disabled",true);
					$("#Band_Steering_En").prop("disabled",true);
			...
			function onloadCheck() {
				if (hide_wifi == 0) {
					onloadCheck_wifi();
					$("#wanpppoe_div").show(); /* ONTS-508 */
				}
				else {
					$("#wifi_div").hide();
					$("#wanpppoe_div").hide(); /* ONTS-508 */
				}
				onloadCheck_account();
				onloadCheck_wan();
			}
			function doSubmit() {
				...
				if (hide_wifi == 0) {
					if (has_config_bndstr()) {
						...
						if ($("#Band_Steering_En").val() == "1") {
							$('input[name="SSID5G_bndstr_off"]').val("<%tcWebApi_get("wlan11ac.entry.1","SSID","s")%>");
							$('input[name="AuthMode5G_bndstr_off"]').val("<%tcWebApi_get("wlan11ac.entry.1","AuthMode","s")%>");
							$('input[name="WPAPSK5G_bndstr_off"]').val("<%tcWebApi_get("wlan11ac.entry.1","WPAPSK","s")%>");
						}
						else {
							var cur_wlssid = $("#ESSID").val();
							if ($('input[name="SSID5G_bndstr_off"]').val() == "N/A") {
								$('input[name="SSID5G_bndstr_off"]').val("<%tcWebApi_get("wlan11ac.entry.1","SSID","s")%>");
							}
							else {
								$('input[name="SSID5G_bndstr_off"]').val($("#ESSID11AC").val());
							}
							if ($("#AuthMode5G_bndstr_off").val() == "N/A"){
								$('input[name="AuthMode5G_bndstr_off"]').val("<%tcWebApi_get("wlan11ac.entry.1","AuthMode","s")%>");
							}
							else {
								$('input[name="AuthMode5G_bndstr_off"]').val(getSelectVal("WEP_Selection_11ac"))
							}
							if ($("#WPAPSK5G_bndstr_off").val() == "N/A"){
								$('input[name="WPAPSK5G_bndstr_off"]').val("<%tcWebApi_get("wlan11ac.entry.1","WPAPSK","s")%>");
							}
							else {
								$('input[name="WPAPSK5G_bndstr_off"]').val($("#PreSharedKey11AC").val())
							}
							if (cur_wlssid == $('input[name="SSID5G_bndstr_off"]').val()) {
								$("#SSID5G_bndstr_off").val(cur_wlssid + "_5G");
							}
						}
					}
					if (has_config_wifi()) {
				...
				if (has_config_account()) {
					...
					var curr_password = $('input[name="passwd"]').val();
					var web_password = "<% tcWebApi_get("Account_Entry0", "web_passwd", "s") %>"
					if (password.length == 0) {
						alert("<%tcWebApi_get("String_Entry","AdminJS0Text","s")%>");
						return false;
					}
					if(curr_password != web_password)
					{
						alert("<%tcWebApi_get("String_Entry","WrongPasswd","s")%>");
						return false;
					}
			...
			function ShowBandSteering(onload) {
				if ($("#Band_Steering_En").prop("checked") == true) {
					...
					if ((onload == 0)&( $("#ESSID").val() == $("#ESSID11AC").val())) {
						var ssid_5g = "<%tcWebApi_get("wlan11ac.entry.1","SSID5G_bndstr_off","s")%>";
						var auth_5g = "<%tcWebApi_get("wlan11ac.entry.1","AuthMode5G_bndstr_off","s")%>";
						var pass_5g = "<%tcWebApi_get("wlan11ac.entry.1","WPAPSK5G_bndstr_off","s")%>";
						if (ssid_5g == "N/A" || (ssid_5g == $("#ESSID").val())) ssid_5g = $("#ESSID").val() + "_5G";
						if (auth_5g == "N/A") auth_5g = $("#WEP_Selection_11ac").val()
						if (pass_5g == "N/A") pass_5g = $("#PreSharedKey11AC").val()
						$("#ESSID11AC").val(ssid_5g);
						setSelect("WEP_Selection_11ac", auth_5g)
						$("#PreSharedKey11AC").val(pass_5g);
					}
					if (onload == 0) {
						if (wAPEnable != "1") {
							$("#HaveWifi-div").hide();
...
<header class="fixed-top"><nav id="top-nav" class="navbar navbar-expand-lg container"><a class="navbar-brand " href="/cgi-bin/home.asp"><div><img src="/img/logo_home.png" width="100" height="18" alt=""><div style="font-family: sans-serif; font-weight: bold;font-size: 12px;color: #000; margin-top: 0">OPTICAL NETWORK TERMINATION</div></div></a><button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarCollapse" aria-controls="navbarCollapse" aria-expanded="false" aria-label="Toggle navigation"><span class="navbar-toggler-icon"><em class="fas fa-align-justify"></em></span></button><div class="collapse navbar-collapse" id="navbarCollapse"><ul class="navbar-nav mr-auto sidenav" id="navAccordion"><li class="nav-item active"><a class="nav-link" href="/cgi-bin/home.asp"><%tcWebApi_get("String_Entry","NavigationHomeText","s")%><span class="sr-only">(current)</span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/deviceinfo.asp"><%tcWebApi_get("String_Entry","NavigationStatusDeviceText","s")%><span class="sr-only"></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/wifi-mesh-topology.asp"><%tcWebApi_get("String_Entry","EasyMeshStatus5Text","s")%><span class="sr-only"></span></a></li><li class="nav-item"><a class="nav-link nav-link-collapse" href="#" id="hasSubItems" data-toggle="collapse" data-target="#status-menu" aria-controls="status-menu" aria-expanded="false"><%tcWebApi_get("String_Entry","NavigationStatusStatisticsText","s")%></a><ul class="nav-second-level collapse" id="status-menu" data-parent="#navAccordion"><li class="nav-item"><a class="nav-link" href="/cgi-bin/sta-statistics.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","NavigationTrafficText","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/sta-wifiusers.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","NavigationStatusUsersText","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/sta-users.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","ARPStatusText","s")%></span></a></li></ul></li><li class="nav-item"><a class="nav-link nav-link-collapse" href="#" id="hasSubItems" data-toggle="collapse" data-target="#network-menu" aria-controls="network-menu" aria-expanded="false"><%tcWebApi_get("String_Entry","NAVNetwork","s")%></a><ul class="nav-second-level collapse" id="network-menu" data-parent="#navAccordion"><li class="nav-item"><a class="nav-link" href="/cgi-bin/net-wan.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","NavigationBasInternetText","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/net-lan.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","NavigationBasLANText","s")%></span></a></li></ul></li><li class="nav-item"><a class="nav-link nav-link-collapse" href="#" id="hasSubItems" data-toggle="collapse" data-target="#wifi-menu" aria-controls="wifi-menu" aria-expanded="false"><%tcWebApi_get("String_Entry","NAVWifi","s")%></a><ul class="nav-second-level collapse" id="wifi-menu" data-parent="#navAccordion"><li class="nav-item"><a class="nav-link" href="/cgi-bin/wifi.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","NavigationWiFiSettingsText","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/wifi-11ac.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","NavigationWiFi11acSettingsText","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/wifi-mesh.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","NavigationMesh","s")%></span></a></li></ul></li><li class="nav-item"><a class="nav-link nav-link-collapse" href="#" id="hasSubItems" data-toggle="collapse" data-target="#firewall-menu" aria-controls="firewall-menu" aria-expanded="false"><%tcWebApi_get("String_Entry","NavigationAdvFirewallText","s")%></a><ul class="nav-second-level collapse" id="firewall-menu" data-parent="#navAccordion"><li class="nav-item"><a class="nav-link" href="/cgi-bin/fw-spi.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","ContentFirewallText","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/fw-urlfilter.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","FILTERTYPE_index2","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/fw-macfilter.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","NAVL2Filter","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/fw-parentalcontrol.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","ParentalControlText","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/safehome.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","SafehomeText","s")%></span></a></li></ul></li><li class="nav-item"><a class="nav-link nav-link-collapse" href="#" id="hasSubItems" data-toggle="collapse" data-target="#apps-menu" aria-controls="apps-menu" aria-expanded="false"><%tcWebApi_get("String_Entry","NAVApplication","s")%></a><ul class="nav-second-level collapse" id="apps-menu" data-parent="#navAccordion"><li class="nav-item"><a class="nav-link" href="/cgi-bin/apps-dmz.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","NatTopDMZText","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/apps-portforwarding.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","NatVirsvrText","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/apps-sslim.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","SessionLimit","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/apps-route.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","StaticRouteText","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/apps-ddns.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","dyDNS1Text","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/apps-wifischeduling.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","WifiSchedulingText","s")%></span></a></li></ul></li><li class="nav-item"><a class="nav-link nav-link-collapse" href="#" id="hasSubItems" data-toggle="collapse" data-target="#tools-menu" aria-controls="tools-menu" aria-expanded="false"><%tcWebApi_get("String_Entry","NavigationMaintenanceText","s")%></a><ul class="nav-second-level collapse" id="tools-menu" data-parent="#navAccordion"><li class="nav-item"><a class="nav-link" href="/cgi-bin/tools-admin.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","NavigationMainAdminText","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/tools-time.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","NAVSysTime","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/tools-firmware.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","NavigationMainFirmwareText","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/tools-config.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","NAVConfig","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/tools-diagnostic.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","NavigationMainDiagnosticsText","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/tools-wan2lan.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","Wan2lanText","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/tools-log.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","NAVLogs","s")%></span></a></li><li class="nav-item"><a class="nav-link" href="/cgi-bin/tools-reboot.asp"><span class="nav-link-text"><%tcWebApi_get("String_Entry","RebootText","s")%></span></a></li></ul></li></ul><div class="top-item-groups"><form name="LanguageForm" method="POST" action="/cgi-bin/home.asp" class="nav-item dropdown"><input type="hidden" name="TokenString" value="<%tcWebApi_getToken()%>"><input type="hidden" name="Language_Flag" value="<% tcWebApi_get("LanguageSwitch_Entry","Type","s") %>"><a class="nav-link dropdown-toggle" href="#" id="LanguageDropdown" role="button" data-toggle="dropdown" aria-expanded="false"><% tcWebApi_get("String_Entry","LoginLanguage","s") %></a><ul class="dropdown-menu" aria-labelledby="LanguageDropdown"><li><a class="dropdown-item <% if tcWebApi_get("LanguageSwitch_Entry","Type","h") = "1" then asp_Write("active") end if %>" href="#" onclick="submitLanguageForm(1);"><% tcWebApi_get("String_Entry","LanguageVIELong","s") %></a></li><li><a class="dropdown-item <% if tcWebApi_get("LanguageSwitch_Entry","Type","h") = "2" then asp_Write("active") end if %>" href="#" onclick="submitLanguageForm(2);"><% tcWebApi_get("String_Entry","LanguageENGLong","s") %></a></li></ul></form><div id="signout" class="nav-item"><a class="nav-link" href="javascript:doLogout();"><em class="fa fa-sign-out-alt"></em><span class="nav-link-text"><% tcWebApi_get("String_Entry","LogoutText","s") %></span></a></div></div></div></header></nav><% if tcWebApi_get("Mesh_dat", "MapEnable", "h") = "1" then
	if tcWebApi_get("Mesh_Common", "DeviceRole", "h") = "2" then %>
...
<% end if end if %>		<div id="content">
			...
			<input type="hidden" id="SSID5G_bndstr_off" name="SSID5G_bndstr_off" value="<%tcWebApi_get("wlan11ac.entry.1","SSID5G_bndstr_off","s")%>" > 
			<input type="hidden" id="AuthMode5G_bndstr_off" name="AuthMode5G_bndstr_off" value="<%tcWebApi_get("wlan11ac.entry.1","AuthMode5G_bndstr_off","s")%>">
			<input type="hidden" id="WPAPSK5G_bndstr_off" name="WPAPSK5G_bndstr_off" value="<%tcWebApi_get("wlan11ac.entry.1","WPAPSK5G_bndstr_off","s")%>">
			...
			<div class="row row-w-100">
								...
								<div class="form-group">
									...
									<div class="col-lg-6 custom-select">
										<select id="WEP_Selection" name="WEP_Selection" onChange="doWEPChange()">
											...
											<!-- <OPTION value="WPAPSK" <% if tcWebApi_get("WLan_Entry0","AuthMode","h") = "WPAPSK" then asp_Write("selected") end if %> <% if tcWebApi_get("Mesh_dat", "MapEnable", "h") = "1" then asp_Write ("disabled") end if %> >WPA-PSK -->
											<OPTION value="WPAPSK" <% if tcWebApi_get("WLan_Entry0","AuthMode","h") = "WPAPSK" then asp_Write("selected") end if %> >WPA-PSK
											...
											<OPTION value="WPAPSKWPA2PSK" <% if tcWebApi_get("WLan_Entry0","AuthMode","h") = "WPAPSKWPA2PSK" then asp_Write("selected") end if %> >WPA/WPA2-PSK
										</select>
									</div>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									...
									<div class="col-lg-6 custom-select">
										<select id="WEP_Selection_11ac" name="WEP_Selection_11ac" onChange="doWEPChange_11ac()">
											...
											<!-- [ONTS-772]: Here we enable all authmode in case mesh is enabled -->
											<!-- <OPTION value="WPAPSK" <% if tcWebApi_get("WLan11ac_Entry","AuthMode","h") = "WPAPSK" then asp_Write("selected") end if %> <% if tcWebApi_get("Mesh_dat", "MapEnable", "h") = "1" then asp_Write ("disabled") end if %> >WPA-PSK -->
											<OPTION value="WPAPSK" <% if tcWebApi_get("WLan11ac_Entry","AuthMode","h") = "WPAPSK" then asp_Write("selected") end if %> >WPA-PSK
											...
											<OPTION value="WPAPSKWPA2PSK" <% if tcWebApi_get("WLan11ac_Entry0","AuthMode","h") = "WPAPSKWPA2PSK" then asp_Write("selected") end if %>>WPA/WPA2-PSK
										</select>
									</div>
								</div>
								<div class="form-group">
				...
				<div class="widget col-xs-12 col-lg-6" id="wanpppoe_div">
