...
%>
<%
if Request_Form("SafeHome_Flag") = "1" then 
	TCWebApi_set("vSafes_Common","Enable","EnableSafeHome_Flag")
	TCWebApi_commit("vSafes")
	TCWebApi_save()
end if
%>
<!doctype html>
<html>
	<head>
<title><%tcWebApi_get("String_Entry","BranchName","s")%> ONT - <%tcWebApi_get("String_Entry","SafehomeText","s")%></title>		<meta charset="utf-8">
		...
		<script language="javascript" type=text/javascript>
			...
			function SubmitForm()
			{
				...
				if (Form.EnableSafeHome_Flag.value != Form.InitEnableSafeHome_Flag.value) {
					console.log("do submit");
					Form.SafeHome_Flag.value = "1";
				}
				else {
					console.log("dont submit");
					Form.SafeHome_Flag.value = "0";
				}
				submitAsync("ConfigForm");
			}
			function triggerEnable()
			{
				var attackChecked = getCheckVal('enableSafeHome');
				if (attackChecked == '1')
				{
					document.ConfigForm.EnableSafeHome_Flag.value = "Yes";
				}
				else
				{
					document.ConfigForm.EnableSafeHome_Flag.value = "No";
				}
			}
			// function get_status() {
			// 	$.ajax({
			// 		url: "/cgi-bin/get_safehome.cgi",
			// 		method: 'GET', 
			// 		complete: function(res){
			// 			try {
			// 				if (/iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream) {
			...
			// 				console.log(obj)
			// 				if(obj["Enable"] == "Yes") {
			// 					document.getElementById("connect_status").style.color   = "black";
			// 					document.getElementById("connect_status").innerText = '<%tcWebApi_get("String_Entry","SafehomeConnText","s")%>';
			// 				}
			// 				else {
			// 					document.getElementById("connect_status").style.color = "red";
			// 					document.getElementById("connect_status").innerText = '<%tcWebApi_get("String_Entry","SafehomeUnconnText","s")%>';
			// 				}
			// 			}
			// 			catch {
			// 			}
...
<% end if end if %>		<div id="content">
			<form class="form-inline" METHOD="POST" ACTION="/cgi-bin/safehome.asp" name="ConfigForm" id="ConfigForm" >
			<input type="hidden" name="TokenString" value="<%tcWebApi_getToken()%>">
			<input type="hidden" id="SafeHome_Flag" name="SafeHome_Flag" value="0">
			<input type="hidden" id="InitEnableSafeHome_Flag" name="InitEnableSafeHome_Flag" value="<% tcWebApi_get("vSafes_Common","Enable","s") %>">
        	<input type="hidden" id="EnableSafeHome_Flag" name="EnableSafeHome_Flag" value="<% tcWebApi_get("vSafes_Common","Enable","s") %>">
			<div class="row row-w-100">
				<div class="widget col-sm-12">
					<header><%tcWebApi_get("String_Entry","ContentSafeHomeText","s")%></header> 
					<div class="widget-body">
						<div class="form-group">
							<label class="col-sm-5"><%tcWebApi_get("String_Entry","EnableText","s")%></label>
							<label class="toggle-switch">
								<input type="checkbox" name="enableSafeHome" id="enableSafeHome" onclick="triggerEnable()" <%if tcWebApi_get("vSafes_Common","Enable","h") = "Yes" then asp_Write("checked") end if%>>
